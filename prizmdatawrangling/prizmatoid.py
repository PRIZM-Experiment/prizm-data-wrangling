...
def interpolate_short(prizm_data, antenna='100MHz', polarization='EW', trim = (1,1) ):
    """
    """

    # Number of frequency channels.
    nfreq = prizm_data[antenna][polarization]['pol'].shape[1]

    select_antenna = (shrink_flag(prizm_data[antenna][polarization]['Flags']['antenna'], trim) == 1)
    select_short = (shrink_flag(prizm_data[antenna][polarization]['Flags']['short'], trim) == 1)

    full_data = prizm_data[antenna][polarization]['pol']
    antenna_data = prizm_data[antenna][polarization]['pol'][select_antenna]
    short_data = prizm_data[antenna][polarization]['pol'][select_short]
    ...
    averaged_time = (prizm_data[antenna][polarization]['time_sys_start'] + prizm_data[antenna][polarization]['time_sys_stop'])/2.
...
def get_closest_slices_in_time(prizm_data, target_slice, list_slices, ctime_distance_threshold=2000, num_slices=2, antenna='100MHz', polarization='EW'):
    """
    """
    ...
    distances = np.array([
                 [np.abs(prizm_data[antenna][polarization]['time_sys_start'][entry_slice].mean() - prizm_data[antenna][polarization]['time_sys_start'][target_slice].mean()), entry_slice]
...
def get_temps_from_ctimes(prizm_data, temp_file, antenna, ctime_range):
    """
    """

    # Converts the input `ctime_range` into a slice object.
    target_slice = slice(np.where(prizm_data[antenna]['time_sys_start.raw'] >= ctime_range[0])[0][0],
                         np.where(prizm_data[antenna]['time_sys_start.raw'] <= ctime_range[1])[0][-1] + 1,
                         None
                         )
